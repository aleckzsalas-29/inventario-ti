...
class EquipmentFieldCreate(BaseModel):
    name: str
...
class EquipmentFieldResponse(BaseModel):
    model_config = ConfigDict(extra="ignore")
    id: str
    name: str
    field_type: str
...
class EquipmentCreate(BaseModel):
    ...
    model: str
    serial_number: str
    acquisition_type: str  # Propio, Arrendado, Prestamo
    acquisition_date: Optional[str] = None
    provider: Optional[str] = None
    status: str = "Disponible"  # Disponible, Asignado, En Reparacion, De Baja
...
class EquipmentLogCreate(BaseModel):
    equipment_id: str
    log_type: str  # Mantenimiento, Incidencia, Cambio, Nota
    description: str
...
class RepairResponse(BaseModel):
    ...
    service_provider: Optional[str] = None
    cost: float = 0.0
    exit_observations: Optional[str] = None
    status: str  # En Proceso, Finalizada
...
class ExternalServiceCreate(BaseModel):
    company_id: str
    service_type: str  # Hosting, Servidor Privado, Dominio, SSL, Cloud, etc.
    provider: str
    description: Optional[str] = None
    start_date: str
    renewal_date: Optional[str] = None
    cost: Optional[float] = 0.0
    payment_frequency: Optional[str] = None  # Mensual, Anual, etc.
...
class QuotationResponse(BaseModel):
    ...
    notes: Optional[str] = None
    valid_until: str
    status: str  # Pendiente, Aceptada, Rechazada, Convertida
    created_at: str
...
class InvoiceResponse(BaseModel):
    ...
    total: float
    notes: Optional[str] = None
    status: str  # Pendiente, Pagada, Anulada
    created_at: str
...
async def init_default_roles():
    default_roles = [
        {
            ...
            "permissions": ["admin", "users.read", "users.write", "companies.read", "companies.write", 
                          ...
                          "repairs.read", "repairs.write", "services.read", "services.write",
        ...
        },
        {
            ...
            "permissions": ["equipment.read", "equipment.write", "assignments.read", "assignments.write",
                          "repairs.read", "repairs.write", "services.read"],
            "description": "Gestión de equipos y reparaciones",
            "is_system": True
        },
        {
            "id": "role-client",
    ...
    if not admin_exists:
        ...
        logger.info("Default admin user created: admin@inventarioti.com / admin123")

@app.on_event("startup")
async def startup_event():
    await init_default_roles()
    logger.info("Application started, default roles initialized")
...
async def create_employee(emp_data: EmployeeCreate, current_user: dict = Depends(get_current_user)):
    ...
    if existing:
        raise HTTPException(status_code=400, detail="Ya existe un empleado con ese DNI en la empresa")
...
# ==================== EQUIPMENT FIELDS ENDPOINTS ====================

@api_router.get("/equipment-fields", response_model=List[EquipmentFieldResponse])
async def get_equipment_fields(current_user: dict = Depends(get_current_user)):
    fields = await db.equipment_fields.find({"is_active": {"$ne": False}}, {"_id": 0}).to_list(100)
    return [EquipmentFieldResponse(**f) for f in fields]

@api_router.post("/equipment-fields", response_model=EquipmentFieldResponse)
async def create_equipment_field(field_data: EquipmentFieldCreate, current_user: dict = Depends(get_current_user)):
    ...
    await db.equipment_fields.insert_one(field)
    return EquipmentFieldResponse(**field)

@api_router.put("/equipment-fields/{field_id}", response_model=EquipmentFieldResponse)
async def update_equipment_field(field_id: str, field_data: EquipmentFieldCreate, current_user: dict = Depends(get_current_user)):
    await check_permission(current_user, "equipment.write")
    
    update_data = field_data.model_dump()
    result = await db.equipment_fields.update_one({"id": field_id}, {"$set": update_data})
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Campo no encontrado")
    
    field = await db.equipment_fields.find_one({"id": field_id}, {"_id": 0})
    return EquipmentFieldResponse(**field)

@api_router.delete("/equipment-fields/{field_id}")
async def delete_equipment_field(field_id: str, current_user: dict = Depends(get_current_user)):
    await check_permission(current_user, "equipment.write")
    result = await db.equipment_fields.update_one({"id": field_id}, {"$set": {"is_active": False}})
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Campo no encontrado")
    return {"message": "Campo desactivado"}
...
async def create_equipment(eq_data: EquipmentCreate, current_user: dict = Depends(get_current_user)):
    ...
    equipment = {
        ...
        "branch_id": eq_data.branch_id,
        "inventory_code": eq_data.inventory_code,
        "equipment_type": eq_data.equipment_type,
        "brand": eq_data.brand,
        "model": eq_data.model,
        "serial_number": eq_data.serial_number,
        "acquisition_type": eq_data.acquisition_type,
        "acquisition_date": eq_data.acquisition_date,
        "provider": eq_data.provider,
        "status": eq_data.status,
        "observations": eq_data.observations,
        "custom_fields": eq_data.custom_fields,
        "assigned_to": None,
        "created_at": now_iso()
    }
...
async def create_assignment(assign_data: AssignmentCreate, current_user: dict = Depends(get_current_user)):
    ...
    if eq.get("status") != "Disponible":
        raise HTTPException(status_code=400, detail="El equipo no está disponible para asignación")
    ...
    assignment = {
        "id": generate_id(),
        "equipment_id": assign_data.equipment_id,
        "employee_id": assign_data.employee_id,
        "delivery_date": assign_data.delivery_date,
        "return_date": None,
        "status": "Activa",
        "observations": assign_data.observations,
...
async def return_assignment(assignment_id: str, observations: Optional[str] = None, current_user: dict = Depends(get_current_user)):
    ...
    return {"message": "Asignación finalizada, equipo devuelto"}

# ==================== REPAIRS ENDPOINTS ====================

@api_router.get("/repairs", response_model=List[RepairResponse])
async def get_repairs(status: Optional[str] = None, current_user: dict = Depends(get_current_user)):
    query = {}
    if status:
        query["status"] = status
    
    repairs = await db.repairs.find(query, {"_id": 0}).sort("entry_date", -1).to_list(1000)
    
    result = []
    for repair in repairs:
        eq = await db.equipment.find_one({"id": repair["equipment_id"]}, {"_id": 0})
        if eq:
            repair["equipment_code"] = eq.get("inventory_code")
        result.append(RepairResponse(**repair))
    return result

@api_router.post("/repairs", response_model=RepairResponse)
async def create_repair(repair_data: RepairCreate, current_user: dict = Depends(get_current_user)):
    await check_permission(current_user, "repairs.write")
    
    eq = await db.equipment.find_one({"id": repair_data.equipment_id})
    if not eq:
        raise HTTPException(status_code=404, detail="Equipo no encontrado")
    if eq.get("status") == "Asignado":
        raise HTTPException(status_code=400, detail="El equipo está asignado, debe devolverse primero")
    ...
    repair = {
        ...
        "entry_date": now_iso(),
        "exit_date": None,
        "reason": repair_data.reason,
        "service_provider": repair_data.service_provider,
        "cost": repair_data.cost or 0.0,
        "exit_observations": None,
        "status": "En Proceso"
    }
    await db.repairs.insert_one(repair)
    
    # Update equipment status
    await db.equipment.update_one(
        {"id": repair_data.equipment_id},
        {"$set": {"status": "En Reparacion"}}
    )
    ...
    log = {
        ...
        "description": f"Enviado a reparación: {repair_data.reason}",
        "performed_by": current_user["id"],
        "created_at": now_iso()
    }
    await db.equipment_logs.insert_one(log)
...
@api_router.put("/repairs/{repair_id}/finish")
async def finish_repair(repair_id: str, exit_observations: Optional[str] = None, cost: Optional[float] = None, current_user: dict = Depends(get_current_user)):
    await check_permission(current_user, "repairs.write")
    
    repair = await db.repairs.find_one({"id": repair_id})
    if not repair:
        raise HTTPException(status_code=404, detail="Reparación no encontrada")
    if repair["status"] != "En Proceso":
        raise HTTPException(status_code=400, detail="La reparación ya fue finalizada")
    ...
    log = {
        ...
        "log_type": "Mantenimiento",
        "description": f"Reparación finalizada. {exit_observations or ''}",
        "performed_by": current_user["id"],
        "created_at": now_iso()
    }
    await db.equipment_logs.insert_one(log)
    
    return {"message": "Reparación finalizada"}
...
async def create_decommission(dec_data: DecommissionCreate, current_user: dict = Depends(get_current_user)):
    ...
    decommission = {
        "id": generate_id(),
        "equipment_id": dec_data.equipment_id,
        "decommission_date": now_iso(),
...
async def create_external_service(svc_data: ExternalServiceCreate, current_user: dict = Depends(get_current_user)):
    ...
    service = {
        "id": generate_id(),
        "company_id": svc_data.company_id,
        "service_type": svc_data.service_type,
        "provider": svc_data.provider,
        "description": svc_data.description,
        "start_date": svc_data.start_date,
        "renewal_date": svc_data.renewal_date,
        "cost": svc_data.cost or 0.0,
        "payment_frequency": svc_data.payment_frequency,
        "credentials_info": svc_data.credentials_info,
        "is_active": True,
        "created_at": now_iso()
    }
...
async def create_quotation(quot_data: QuotationCreate, current_user: dict = Depends(get_current_user)):
    ...
    quotation = {
        ...
        "company_id": quot_data.company_id,
        "client_name": quot_data.client_name,
        "client_email": quot_data.client_email,
        "client_address": quot_data.client_address,
        "items": items,
        "subtotal": round(subtotal, 2),
        "tax_rate": quot_data.tax_rate,
...
async def create_invoice(inv_data: InvoiceCreate, current_user: dict = Depends(get_current_user)):
    ...
    invoice = {
        ...
        "quotation_id": inv_data.quotation_id,
        "client_name": inv_data.client_name,
        "client_email": inv_data.client_email,
        "client_address": inv_data.client_address,
        "client_tax_id": inv_data.client_tax_id,
        "items": items,
        "subtotal": round(subtotal, 2),
...
async def get_dashboard_stats(current_user: dict = Depends(get_current_user)):
    ...
    eq_filter_available = {**company_filter, "status": "Disponible"}
    available_equipment = await db.equipment.count_documents(eq_filter_available)
    
    eq_filter_assigned = {**company_filter, "status": "Asignado"}
    assigned_equipment = await db.equipment.count_documents(eq_filter_assigned)
    
    eq_filter_repair = {**company_filter, "status": "En Reparacion"}
    repair_equipment = await db.equipment.count_documents(eq_filter_repair)
    
    eq_filter_decommissioned = {**company_filter, "status": "De Baja"}
    decommissioned_equipment = await db.equipment.count_documents(eq_filter_decommissioned)
    ...
    active_repairs = await db.repairs.count_documents({"status": "En Proceso"})
    ...
    # Equipment by type
    pipeline = [
        {"$match": company_filter} if company_filter else {"$match": {}},
...
async def generate_equipment_report_pdf(
    company_id: Optional[str] = None,
    status: Optional[str] = None,
    current_user: dict = Depends(get_current_user)
):
...
async def generate_equipment_logs_pdf(equipment_id: str, current_user: dict = Depends(get_current_user)):
    ...
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 16)
    pdf.cell(0, 10, f"Bitacora de Equipo: {eq.get('inventory_code', '')}", ln=True, align="C")
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 8, f"Tipo: {eq.get('equipment_type', '')} | Marca: {eq.get('brand', '')} | Modelo: {eq.get('model', '')}", ln=True, align="C")
    pdf.cell(0, 8, f"Serie: {eq.get('serial_number', '')} | Estado: {eq.get('status', '')}", ln=True, align="C")
    pdf.ln(10)
...
async def generate_quotation_pdf(quotation_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if company:
        ...
        if company.get("address"):
            pdf.cell(0, 6, company.get("address", ""), ln=True)
        if company.get("phone"):
            pdf.cell(0, 6, f"Tel: {company.get('phone', '')}", ln=True)
    pdf.ln(5)
    ...
    if quot.get("client_address"):
        pdf.cell(0, 6, quot.get("client_address", ""), ln=True)
    if quot.get("client_email"):
        pdf.cell(0, 6, quot.get("client_email", ""), ln=True)
    pdf.ln(10)
    ...
    for item in quot.get("items", []):
        ...
        pdf.cell(20, 7, f"{item.get('discount', 0)}%", 1, align="C")
    ...
    pdf.ln(5)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(150, 7, "Subtotal:", 0, align="R")
    pdf.cell(35, 7, f"${quot.get('subtotal', 0):.2f}", 0, align="R")
    pdf.ln()
    pdf.cell(150, 7, f"Impuesto ({quot.get('tax_rate', 0)}%):", 0, align="R")
    pdf.cell(35, 7, f"${quot.get('tax_amount', 0):.2f}", 0, align="R")
    pdf.ln()
    ...
    # Notes and validity
    pdf.set_font("Helvetica", "", 9)
    valid_date = quot.get("valid_until", "")[:10]
    pdf.cell(0, 6, f"Valido hasta: {valid_date}", ln=True)
    if quot.get("notes"):
        pdf.ln(3)
        pdf.multi_cell(0, 5, f"Notas: {quot.get('notes', '')}")
...
async def generate_invoice_pdf(invoice_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if company:
        ...
        if company.get("tax_id"):
            pdf.cell(0, 6, f"RUC/NIT: {company.get('tax_id', '')}", ln=True)
        if company.get("address"):
            pdf.cell(0, 6, company.get("address", ""), ln=True)
    pdf.ln(5)
    ...
    if inv.get("client_tax_id"):
        pdf.cell(0, 6, f"RUC/NIT: {inv.get('client_tax_id', '')}", ln=True)
    ...
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(150, 7, "Subtotal:", 0, align="R")
    pdf.cell(35, 7, f"${inv.get('subtotal', 0):.2f}", 0, align="R")
    pdf.ln()
    pdf.cell(150, 7, f"Impuesto ({inv.get('tax_rate', 0)}%):", 0, align="R")
    pdf.cell(35, 7, f"${inv.get('tax_amount', 0):.2f}", 0, align="R")
    pdf.ln()
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(150, 8, "TOTAL:", 0, align="R")
...
async def send_email_notification(request: EmailNotificationRequest, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return {"status": "success", "message": f"Email enviado a {request.recipient_email}", "email_id": email.get("id")}
...
async def get_available_permissions(current_user: dict = Depends(get_current_user)):
    return {
        "permissions": [
            {"key": "admin", "label": "Administrador Total", "description": "Acceso completo al sistema"},
            {"key": "users.read", "label": "Ver Usuarios", "description": "Puede ver la lista de usuarios"},
            {"key": "users.write", "label": "Gestionar Usuarios", "description": "Puede crear, editar y eliminar usuarios"},
            {"key": "companies.read", "label": "Ver Empresas", "description": "Puede ver empresas y sucursales"},
            {"key": "companies.write", "label": "Gestionar Empresas", "description": "Puede crear, editar y eliminar empresas"},
            ...
            {"key": "assignments.read", "label": "Ver Asignaciones", "description": "Puede ver las asignaciones de equipos"},
            {"key": "assignments.write", "label": "Gestionar Asignaciones", "description": "Puede asignar y devolver equipos"},
            {"key": "repairs.read", "label": "Ver Reparaciones", "description": "Puede ver las reparaciones"},
            {"key": "repairs.write", "label": "Gestionar Reparaciones", "description": "Puede registrar y finalizar reparaciones"},
            {"key": "services.read", "label": "Ver Servicios Externos", "description": "Puede ver servicios contratados"},
            {"key": "services.write", "label": "Gestionar Servicios", "description": "Puede administrar servicios externos"},
            ...
            {"key": "reports.read", "label": "Ver Reportes", "description": "Puede ver reportes"},
            {"key": "reports.export", "label": "Exportar Reportes", "description": "Puede exportar reportes a PDF/Excel"}
        ]
    }
